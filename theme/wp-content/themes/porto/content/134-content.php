<div class="type-134">
    <div class="blog-section">
        <div class="container">
            <h2 class="h3 title text-center">From the Blog</h2>
            <div class="swiper-container swiper-container-206">
                <div class="swiper-wrapper"
                    style="transform: translate3d(0px, 0px, 0px); transition: 0s; width: 1800px;">
                    <div class="swiper-slide" style="width: 570px; margin-right: 30px;">
                        <article class="entry">
                            <div class="entry-media">
                                <a href="#">
                                    <img src="<?php bloginfo('template_url'); ?>/images/post-1.png" alt="Post">
                                </a>
                                <div class="entry-date">29<span>Now</span></div><!-- End .entry-date -->
                            </div><!-- End .entry-media -->

                            <div class="entry-body">
                                <h3 class="entry-title">
                                    <a href="#">New Collection</a>
                                </h3>
                                <div class="entry-content">
                                    <p>Lorem Ipsum is simply dummy text of the printing and typesetting industry.
                                        Lorem Ipsum has...</p>

                                    <a href="#" class="btn btn-dark">Read More</a>
                                </div><!-- End .entry-content -->
                            </div><!-- End .entry-body -->
                        </article>
                    </div>
                    <div class="swiper-slide" style="width: 570px; margin-right: 30px;">
                        <article class="entry">
                            <div class="entry-media">
                                <a href="#">
                                    <img src="<?php bloginfo('template_url'); ?>/images/post-2.png" alt="Post">
                                </a>
                                <div class="entry-date">30 <span>Now</span></div><!-- End .entry-date -->
                            </div><!-- End .entry-media -->

                            <div class="entry-body">
                                <h3 class="entry-title">
                                    <a href="#">Fashion Trends</a>
                                </h3>
                                <div class="entry-content">
                                    <p>Lorem Ipsum is simply dummy text of the printing and typesetting industry.
                                        Lorem Ipsum has...</p>

                                    <a href="#" class="btn btn-dark">Read More</a>
                                </div><!-- End .entry-content -->
                            </div><!-- End .entry-body -->
                        </article>
                    </div>
                    <div class="swiper-slide" style="width: 570px; margin-right: 30px;">
                        <article class="entry">
                            <div class="entry-media">
                                <a href="#">
                                    <img src="<?php bloginfo('template_url'); ?>/images/post-3.png" alt="Post">
                                </a>
                                <div class="entry-date">28 <span>Now</span></div><!-- End .entry-date -->
                            </div><!-- End .entry-media -->

                            <div class="entry-body">
                                <h3 class="entry-title">
                                    <a href="#">Women Fashion</a>
                                </h3>
                                <div class="entry-content">
                                    <p>Lorem Ipsum is simply dummy text of the printing and typesetting industry.
                                        Lorem Ipsum has...</p>

                                    <a href="#" class="btn btn-dark">Read More</a>
                                </div><!-- End .entry-content -->
                            </div><!-- End .entry-body -->
                        </article>
                    </div>
                </div>
            </div>
        </div><!-- End .blog-carousel -->
        <!-- Add Pagination -->
    </div><!-- End .container -->
</div>
</div>